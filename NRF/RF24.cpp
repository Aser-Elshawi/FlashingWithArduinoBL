...
void RF24::csn(bool mode)
{
    ...
    if (ce_pin != csn_pin) {
        ...
        if (mode == HIGH) {
            ...
            _delay_us(RF24_CSN_SETTLE_HIGH_DELAY); // allow csn to settle.
        }
        else {
            PORTB &= ~(1 << PINB2);                       // SCK->CSN LOW
            _delay_us(RF24_CSN_SETTLE_LOW_DELAY); // allow csn to settle
        }
    }
  ...
  //  SPCR &= ~(1<< DORD);
   ...
   // _spi->setClockDivider(SPI_CLOCK_DIV2);
    ...
    //digitalWrite(csn_pin, mode); aser
    // Porting
    if(mode == HIGH){
    	*(csn_pin.port) |= (1<<csn_pin.pin);
    }else{
      	*(csn_pin.port) &= ~(1<<csn_pin.pin);
    }
    // needs OS porting
...
void RF24::ce(bool level)
{
	...
	// porting
    if (ce_pin.port != csn_pin.port) {
#endif
        //digitalWrite(ce_pin, level);
    	// porting
    	if(level == HIGH){
    		*(ce_pin.port) |= (1<<ce_pin.pin);
    	}else{
    		*(ce_pin.port) &= ~(1<<ce_pin.pin);
    	}
#ifndef RF24_LINUX
...
RF24::RF24(rf24_gpio_pin_t _cepin, rf24_gpio_pin_t _cspin, uint32_t _spi_speed)
    : spi_speed(_spi_speed), payload_size(32), _is_p_variant(false), _is_p0_rx(false), addr_width(5), dynamic_payloads_enabled(true), csDelay(5)
{
	ce_pin.dport = _cepin.dport;
	ce_pin.pin = _cepin.pin;
	ce_pin.port = _cepin.port;
	csn_pin.dport = _cspin.dport;
	csn_pin.pin = _cspin.pin;
	csn_pin.port = _cspin.port;
    _init_obj();
}
...
void RF24::encodeRadioDetails(uint8_t* encoded_details)
{
    ...
    *encoded_details++ = ce_pin.pin >> 4;
    *encoded_details++ = ce_pin.pin & 0xFF;
    *encoded_details++ = csn_pin.pin >> 4;
    *encoded_details++ = csn_pin.pin & 0xFF;
...
bool RF24::_init_pins()
{
    ...
    // Initialize pins
    // Porting
    if ((ce_pin.dport != csn_pin.dport)||(ce_pin.pin != csn_pin.pin)) {
       // pinMode(ce_pin, OUTPUT);
       // pinMode(csn_pin, OUTPUT);
    	*(ce_pin.dport) |= (1<<ce_pin.pin);
    	*(csn_pin.dport) |= (1<< csn_pin.pin);
...
bool RF24::isValid()
{
	return ce_pin.port != 0x0000 && csn_pin.port != 0x0000;
...
void RF24::startConstCarrier(rf24_pa_dbm_e level, uint8_t channel)
{
    ...
    ce(HIGH);
    if (isPVariant()) {
        _delay_ms(1); // datasheet says 1 ms is ok in this instance
        ce(LOW);
        reUseTX();
    }
}
