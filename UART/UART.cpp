...
int UART::getIntLength(int value) {
    int length = 1;
    while (value /= 10) {
        length++;
    }
...
void UART::intToCharArray(int value, char* buffer,int length) {
    ...
    // Convert digits to characters and store in the buffer
    buffer += length;
    *buffer = '\0';
...
UART::UART(long int USART_BAUDRATE){
	init(USART_BAUDRATE);
}
void UART::printd(int value){
	int length = getIntLength(value);
	char buffer[length];
	intToCharArray(value, buffer, length);
	print(buffer);
	print("\n");
}
...
void UART::println(const char* format) {
	// Initialize variable argument list
		const char * newline = "\n";
	    // Process the format string
	    print(format);
...
void UART::init(long USART_BAUDRATE) {
